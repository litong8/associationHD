...
files = list.files(pattern=paste("result1",type,"rho_e=",rho_e,"datagenseed=",seed, "simseed="))
...
write.csv(finalresult,paste("finalresult1",type,"rho_e=",rho_e,"datagenseed=",as.character(seed),".csv"))
}
}

########################################################################################
## create table for publication purpose
## 5 mehthod: "Oracle","ols","Ind","truecor","onestep" 1,3,4,5,6
for (type in c("AR"))
{
if (type=="AR") {rho_eVec=rho_eAR}
else {rho_eVec=rho_eCS}
...
for (Idomain in 1:Q)
{
## select row:Sdomain; column: Idomain five methods 
## 5 mehthod: "Oracle","ols","Ind","truecor","onestep" 1,3,4,5,6
biasIS=bias[(0:(length(Nvec)*length(SNR)-1))*K+Sdomain,(c(0,2:5))*(2+p+Q)+2+p+Idomain]
    SDIS=SD[(0:(length(Nvec)*length(SNR)-1))*K+Sdomain,(c(0,2:5))*(2+p+Q)+2+p+Idomain]
  EseIS=Ese[(0:(length(Nvec)*length(SNR)-1))*K+Sdomain,(c(0,2:5))*(2+p+Q)+2+p+Idomain]
    CRIS=CR[(0:(length(Nvec)*length(SNR)-1))*K+Sdomain,(c(0,2:5))*(2+p+Q)+2+p+Idomain]
...
write.csv(round(resulttable,3),paste("resulttable1",type,"rho_e=",rho_e,"Sdomain=",Sdomain,".csv"))
} # end Sdomain 
...
result=array(data=NA,dim=c(K*4*length(Nvec)*length(SNR),betaNum*(2+p+Q),Batch))
### obtain result from each file
files = list.files(pattern=paste("result2",type,"rho_e=",rho_e,"datagenseed=",seed, "simseed="))
for (i in 1:Batch) {result[,,i]=as.matrix(read.csv(file = files[i],row.names = 1))}
...
outputname=c("Bias","SD","Ese","CR")
rownames(finalresult)=paste(rep(outputname,each=length(SNRNsname)),SNRNsname,sep=" ")
write.csv(finalresult,paste("finalresult2",type,"rho_e=",rho_e,"datagenseed=",as.character(seed),".csv"))
}
}
...
write.csv(round(resulttable,3),paste("resulttable2",type,"rho_e=",rho_e,"Sdomain=",Sdomain,".csv"))
} # end Sdomain 
...
result=array(data=NA,dim=c(K*4*length(Nvec)*length(SNR),betaNum*(2+p+Q),Batch))
### obtain result from each file
files = list.files(pattern=paste("result3",type,"rho_e=",rho_e,"datagenseed=",seed, "simseed="))
for (i in 1:Batch) {result[,,i]=as.matrix(read.csv(file = files[i],row.names = 1))}
...
outputname=c("Bias","SD","Ese","CR")
rownames(finalresult)=paste(rep(outputname,each=length(SNRNsname)),SNRNsname,sep=" ")
write.csv(finalresult,paste("finalresult3",type,"rho_e=",rho_e,"datagenseed=",as.character(seed),".csv"))
}
}
...
write.csv(round(resulttable,3),paste("resulttable3",type,"rho_e=",rho_e,"Sdomain=",Sdomain,".csv"))
} # end Sdomain 
